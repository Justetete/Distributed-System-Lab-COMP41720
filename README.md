...
TestSocketServer - Unit tests for SocketServer class
test_server_initialization
Test server initialisation with default and custom parameters.
test_process_message
Test message processing functionality.
TestSocketClient - unit tests for SocketClient class
test_client_initialization
Test client initialisation with default and custom parameters.
TestIntegration - Integration tests for server-client communication
test_basic_communication
Test basic client-server communication
test_multiple_clients
Test server handling multiple clients simultaneously
test_connection_refused
Test client behaviour when the server is not running.
test_server_restart
Test server can be restarted on the same port
TestErrorConditions - Tests for various error conditions
test_invalid_host
Test client behaviour with an invalid host
test_invalid_port
Test client behaviour with an invalid port
test_connection_timeout
Test client timeout behaviour
test_connection_closed_by_server
Test client behaviour when the server closes the connection
test_malformed_input_handling
Test server handling of malformed input
TestCLIInterface - Tests for command-line interface
test_server_cli_arguments
Test server command-line argument parsing
test_client_cli_arguments
...
Validation Errors: 400: Missing required fields, 409: User with ID already exists
PUT /api/users/<id>
...
User Retrieval
Supports both individual and bulk user retrieval. The show_users() method returns all users as a list, while get_user() retrieves a specific user by ID.
Iterates through the user_lists dictionary
Converts user objects to dictionary format
...
Checks user existence before updating
...
Uses pop() to remove and retrieve user data
Returns deleted user information
Handles non-existent users with 404 status
Error Handling
...
Test 1 - Create a User (POST http://localhost:5000/api/users)
...
Test 2 - Retrieve all users (GET http://localhost:5000/api/users)
Successfully return all users' information, HTTP status code is 200.

Test 3 - Retrieve specific user by ID (GET http://localhost:5000/api/users/4)
Successfully return the user’s information with the ID is 4, HTTP status code is 200

Test 4 - Update User (PUT http://localhost:5000/api/users/1)
...
Test 5 - Delete User (DELETE http://localhost:5000/api/users/2)
...
Client Request → gRPC Stub → Network (HTTP/2) → gRPC Server → Service Logic
                                                                     ↓
Client Response ← gRPC Stub ← Network (HTTP/2) ← gRPC Server ← Return Value
Benefits of gRPC:
Efficient binary serialisation (smaller payload than JSON)
HTTP/2 multiplexing (multiple requests over a single connection)
Built-in code generation (no manual parsing)
...
Input: Name="John Doe", ID=100, Email="john@example.com"
...
Input: Update ID=100, Name="John Smith"
